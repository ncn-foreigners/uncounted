...
For observation $i$, the expected observed count $m_{i}$ is modelled as:

$$E\left( m_{i} \right) = N_{i}^{\alpha_{i}} \cdot \left( \gamma + n_{i}/N_{i} \right)^{\beta_{i}}$$

where $N_{i}$ is the reference (total registered) population, $n_{i}$ is
an auxiliary count (e.g. new registrations), and $\gamma \geq 0$ is an
